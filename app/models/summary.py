...
class ActionItem(BaseModel):
    ...
    owner: Optional[str] = Field(
        default=None, description="Responsável (se identificado)"
    )
    due_date: Optional[str] = Field(
        default=None, description="Prazo sugerido (AAAA-MM-DD ou texto)"
    )
...
class MeetingSummary(BaseModel):
    ...
    key_points: List[str] = Field(
        default_factory=list, description="Principais pontos discutidos"
    )
    decisions: List[str] = Field(default_factory=list, description="Decisões tomadas")
    action_items: List[ActionItem] = Field(
        default_factory=list, description="Ações a serem tomadas"
    )
    insights: List[str] = Field(
        default_factory=list, description="Insights relevantes/métricas/risco"
    )
